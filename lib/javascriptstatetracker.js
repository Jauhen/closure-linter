 ...
 * @constructor
 * @extends {stateTracker.StateTracker}
 */
...
JavaScriptStateTracker.prototype.reset = function() {
    this._scopeDepth = 0;
    this._blockStack = [];
    stateTracker.StateTracker.prototype.reset.call(this);
};
 ...
 * a class.
 * @return {boolean} Whether we are at the top level in the class.
 */
JavaScriptStateTracker.prototype.inTopLevel = function() {
    return this._scopeDepth == this.parenthesesDepth();
};
 ...
 *
 * @return {boolean} True if the token is not enclosed in a block that does not
 ...
 * @return {string} Code block type for current token.
 */
 ...
 * @return {?Token} Starting token of current block. None if not in block.
 */
...
JavaScriptStateTracker.prototype.handleToken = function(token,
    ...
    stateTracker.StateTracker.prototype.handleToken.call(this, token,
            lastNonSpaceToken);
};
