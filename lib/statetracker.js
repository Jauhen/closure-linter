 ...
 *      desired contents.
 * @return {{endToken: Token, contents: string}} The matching ending token
 *      (DOC_END_BRACE or FLAG_ENDING_TYPE) and a string of the contents
 *      between the matching tokens, minus any comment prefixes.
 */
 ...
 * @return {?Token} The token found containing identifier, null otherwise.
 */
 ...
 * @return {boolean} True if documentation may be pulled off the superclass.
 */
 ...
 * @return {DocFlag} The last instance of the given flag type in this doc
 *      comment.
 */
 ...
 *
 * @return {string} The identifier for the token this comment is for.
 */
 ...
 * @return {?Token} The token that is the target of this comment, or null if
 *      there isn't one.
 */
 ...
 * @return {distance: number, edit: Array.<string>} The edit distance,
 *      the edit list.
 */
DocComment.prototype.compareParameters = function(params) {
    var orderedParams = this.orderedParams();
    var sourceLen = orderedParams.length;
    var targetLen = params.length;
    var editLists = [[]];
    var distance = [[]];
    for (var i = 0; i < targetLen + 1; i++) {
        editLists[0].push(_.map(_.range(i), function() {return 'I'}));
        distance[0].push(i);
    }

    for (var j = 1; j < sourceLen + 1; j++) {
        editLists.push([_.map(_.range(j), function() {return 'D'})]);
        distance.push([j]);
    }
    ...
    for (i = 0; i < sourceLen; i++) {
        for (j = 0; j < targetLen; j++) {
            var cost = 1;
            if (orderedParams[i] == params[j]) {
                cost = 0;
            }

            var deletion = distance[i][j + 1] + 1;
            var insertion = distance[i + 1][j] + 1;
            var substitution = distance[i][j] + cost;
            ...
            if (deletion <= insertion && deletion <= substitution) {
                // Deletion is best.
                best = deletion;
                editList = editLists[i][j].slice();
                editList.push('D');

            } else if (insertion <= substitution) {
                // Insertion is best.
                best = insertion;
                editList = editLists[i + 1][j].slice();
                editList.push('I');
                editLists[i + 1].push(editList);
            ...
            editLists[i + 1].push(editList);
            distance[i + 1].push(best);
        }
    }

    return {distance: distance[sourceLen][targetLen],
        edit: editLists[sourceLen][targetLen]};
};
 ...
 * @return {boolean} True if the current token is within a function.
 */
 ...
 * @return {boolean} True if the current token is within a constructor.
 */
 ...
 * @return {boolean} True if the current token is within an interface method.
 */
 ...
 * @return {boolean} True if the current token is within a top level function.
 */
 ...
 * @return {boolean} True if the current token is a function block open.
 */
 ...
 * @return {boolean} True if the current token is a function block close.
 */
 ...
 * @return {boolean} True if the current token is within a block.
 */
 ...
 * @return {boolean} True if the current token is a block close.
 */
 ...
 * @return {boolean} True if the current token is within an object literal.
 */
 ...
 * @return {boolean} True if the current token has an object literal ancestor.
 */
 ...
 * @return {boolean} True if the current token is within parentheses.
 */
 ...
 * @return {number} The number of parenthesis surrounding the token.
 */
 ...
 * @return {number} The number of blocks in which the token is nested.
 */
 ...
 * @return {number} The number of functions in which the token is nested.
 */
 ...
 *
 * @return {Array.<string>} The accumulated input params as an array.
 */
 ...
 * @return {?DocComment} The last plain comment that could be used as
 *      documentation.
 */
 ...
 * @return {?DocComment} The last applicable documentation comment.
 */
 ...
 * @return {boolean} Whether the identifier has been documented yet.
 */
 ...
 * @return {boolean} Whether the current token is in a doc comment.
 */
 ...
 * @return {?DocFlag} The current documentation flags.
 */
 ...
 * @return {JsFunction} The current Function object.
 */
 ...
 * @return {number} The current block depth.
 */
 ...
 *      scope.
 * @return {boolean} True if given string is a variable in current scope.
 */
...
StateTracker.prototype.handleToken = function(token, lastNonSpaceToken) {
    ...
    // Track parentheses depth.
    ...
    } else if (type == Type.END_DOC_COMMENT) {
        this._docComment.endToken = token;
        ...
        if (nextToken) {
            if (nextToken.type == Type.SIMPLE_LVALUE) {
                this._variablesInScope.push(nextToken.values[1]);
            } else {
