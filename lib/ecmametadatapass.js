 ...
 * @returns {?EcmaContext} The former context.
 * @throw {ParseError} If the root context is popped.
 * @private
 */
 ...
 * @returns {EcmaContext} The context object of the given type that was popped.
 * @private
 */
 ...
 * @returns {EcmaContext} The context that should be assigned to the current
 * token, or null if the current context after this method should be used.
 * @throw {ParseError} When the token appears in an invalid context.
 * @private
 */
EcmaMetaDataPass.prototype._processContext = function() {
    ...
    // while above.
        ...
        this._addContext(EcmaContext.VAR);
    } else if (token.isOperator(',')) {
 ...
 * @returns {string} The type of the operator. One of the *_OPERATOR constants
